...
def load_scenario(fd):
    with open(fd, 'r') as f:
        global GRID_SIZE, ROBOT_RADIUS, RECT_OBSTACLES, START, GOAL,TASKBCET, TASKWCET, TASKSMILETSONE
...
class Draw:
    ...
    def draw(self):
        ...
        for idx, paths in enumerate(self.group_paths.values()):
            self.canvas = np.ones((1000, 1920, 3), dtype=np.uint8) * 255
...
if __name__ == '__main__':
    ...
    global START, GOAL, TASKBCET, TASKWCET, TASKSMILETSONE
    standard = False
    # load_scenario(sys.argv[1])
    load_scenario('MAPF_CBS/Scenario/scenario1 copy.yaml')
    if standard is True:
        # show_pos(START)
        r = Simulator()
        r.start()
    else:
        # Store travelling time and  path for each agent to every possible destination
        data = {}
        desired_data ={}
        desired_data.update(TASKBCET)
        desired_data.update(TASKWCET)
        desired_data.update(TASKSMILETSONE)
        # start and goal coordinate from YAML file
        orig_start = START[:]
        orig_goal = GOAL[:]
        # Store possible path
        paths = []
        ...
        for i in range(len(orig_start)):
            for j in range(len(orig_goal)):
                # Uncomment this and comment the second part if you want to visualize the path
                # r = Simulator()
                # r.start()

                # Comment this and uncomment the first part if you only want to display the data.
                START = [orig_start[i]]
                GOAL = [orig_goal[j]]
                # Call cbs-mapf to plan
                p = run_custom()
